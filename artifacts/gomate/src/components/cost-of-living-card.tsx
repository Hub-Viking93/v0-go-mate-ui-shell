...
export function CostOfLivingCard({
  ...
  // hooks/use-currency-conversion.ts for the rationale (frankfurter.app
  // CORS-blocked from the browser). We render the source currency as-is.
  void citizenship
  void getCurrencyFromCitizenship
  void exchangeRate
  void setExchangeRate
  const dataCurrency = data?.currency || "EUR"
  const localSymbol = CURRENCY_SYMBOLS[dataCurrency] || dataCurrency

  const formatPrice = (value: number, _showConversion = true): string => {
    void _showConversion
    return `${localSymbol}${value.toLocaleString()}`
  }
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="p-6">
        ...
        <TabsContent value="indices" className="space-y-4">
          ...
          {[
            ...
            { label: "Restaurant Price Index", value: data.restaurantsIndex, color: "bg-orange-500" },
            { label: "Purchasing Power Index", value: data.localPurchasingPowerIndex, color: "bg-primary" },
          ].map((index) => (
            <div key={index.label} className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">{index.label}</span>
                <Badge variant={(index.value ?? 0) > 100 ? "destructive" : (index.value ?? 0) > 80 ? "secondary" : "default"}>
                  {index.value || "N/A"}
                </Badge>
              </div>
              {(index.value ?? 0) > 0 && (
                <Progress value={Math.min(index.value ?? 0, 150) / 1.5} className="h-2" />
              )}
            </div>
          ))}
        </TabsContent>
