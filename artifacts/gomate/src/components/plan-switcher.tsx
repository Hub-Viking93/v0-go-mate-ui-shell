...
export function PlanSwitcher({
  ...
  // Free users: just show the title with optional rename.
          ...
          <>
            <div className="flex items-center gap-2">
              {currentPlan.destination && (
                <CountryFlag country={currentPlan.destination} size="sm" />
              )}
        ...
        <>
          ...
          >
            <SelectTrigger className="h-9 w-auto min-w-[200px] max-w-[300px] gap-2 text-sm">
              {/* SelectValue echoes the chosen SelectItem (which already
                  renders its destination flag), so a static flag here would
                  duplicate. Just render the value. */}
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {activePlans.map((plan) => (
                <SelectItem key={plan.id} value={plan.id}>
                  <div className="flex items-center gap-2">
                    {plan.destination && (
                      <CountryFlag country={plan.destination} size="sm" />
                    )}
