...
export function UpgradeModal({ open, onOpenChange, currentTier, onUpgradeComplete }: UpgradeModalProps) {
  ...
  async function handleUpgrade(tier: Tier, billingCycle?: string) {
    setUpgrading(tier)
    try {
      const response = await fetch("/api/subscription", {
        ...
        body: JSON.stringify({
          action: "upgrade",
          tier,
          billing_cycle: billingCycle || (tier === "pro_single" ? "one_time" : selectedCycle),
        }),
      })
  ...
  async function handleDowngrade() {
    ...
    try {
      ...
      console.error("[GoMate] Downgrade error:", error)
    } finally {
      setUpgrading(null)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          ...
          <div className={`relative rounded-2xl border p-5 flex flex-col ${
            ...
            {currentTier !== "free" ? (
              ...
              >
                {upgrading === "free" ? "Processing..." : "Downgrade"}
              </Button>
            ) : (
          ...
          <div className={`relative rounded-2xl border p-5 flex flex-col ${
            ...
            {currentTier === "pro_single" ? (
              ...
              <Button
                variant="outline"
                className="w-full rounded-xl bg-transparent"
                onClick={() => handleUpgrade("pro_single", "one_time")}
                disabled={upgrading !== null}
              >
                {upgrading === "pro_single" ? "Processing..." : "Switch to Pro Single"}
              </Button>
            ) : (
              <Button
                ...
                disabled={upgrading !== null}
              >
                {upgrading === "pro_single" ? "Processing..." : "Get Pro Single"}
              </Button>
            )}
          </div>
          ...
          <div className={`relative rounded-2xl border-2 p-5 flex flex-col ${
            ...
            {currentTier === "pro_plus" ? (
              ...
              <Button
                ...
                onClick={() => handleUpgrade("pro_plus", selectedCycle)}
                disabled={upgrading !== null}
              >
                {upgrading === "pro_plus" ? "Processing..." : "Get Pro+"}
              </Button>
            )}
          </div>
        </div>
        ...
        <p className="text-xs text-muted-foreground text-center mt-4">
          Payment integration coming soon. Plans are activated immediately for early access users.
        </p>
      </DialogContent>
    </Dialog>
  )
